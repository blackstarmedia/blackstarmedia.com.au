/**
 * Black Star Media — AI Mega Vault Workflow Email Delivery
 * ==========================================================
 * A Google Apps Script web app that powers the email capture on
 * ai-mega-vault-workflow.html. When a visitor submits their email it:
 ...
 *    Name it "Black Star — AI Mega Vault Workflow Delivery".
...
var CONFIG = {
  ...
  PDF_URL:      "https://blackstarmedia.com.au/assets/pdf/ai-mega-vault-workflow.pdf",
  PDF_FILENAME: "AI-Mega-Vault-Workflow.pdf",
  PDF_TITLE:    "AI Mega Vault Workflow",

  // Leave blank to auto-create a Sheet on first run.
  SHEET_ID:   "",
  SHEET_NAME: "AI Mega Vault Workflow Signups",
...
function doGet() {
  return json_({ ok: true, service: "ai-mega-vault-workflow-delivery" });
}
...
function getSheet_() {
  ...
  var id    = CONFIG.SHEET_ID || props.getProperty("AI_MEGA_VAULT_WORKFLOW_SHEET_ID");
  var ss;
  if (id) {
    ss = SpreadsheetApp.openById(id);
  } else {
    ss = SpreadsheetApp.create("Black Star — AI Mega Vault Workflow Signups");
    props.setProperty("AI_MEGA_VAULT_WORKFLOW_SHEET_ID", ss.getId());
  }
...
function sendEmail_(email) {
  ...
  var downloadLink = CONFIG.SITE_URL + "/assets/pdf/ai-mega-vault-workflow.pdf";
